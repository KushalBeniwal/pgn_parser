...
def make_normal_move(piece, move_to, board_view, piece_view):
    if (
        move_to[-1] == REGULAR_DOUBLE_MOVE_TO[piece]
        and board_view[move_to[0] + REGULAR_DOUBLE_MOVE_FROM[piece]] == piece
    ):
...
def make_capture(piece, move_to, move_from, board_view, piece_view):
    if move_from == None:
        if (
            chr(ord(move_to[0]) + 1)
            + chr(ord(move_to[1]) + (1 if piece.isupper() else -1))
            in piece_view[piece]
        ):
            move_from = chr(ord(move_to[0]) + 1) + chr(
                ord(move_to[1]) + (1 if piece.isupper() else -1)
            )
        else:
            move_from = chr(ord(move_to[0]) - 1) + chr(
                ord(move_to[1]) + (1 if piece.isupper() else -1)
...
def make_en_passant(piece, move_to, move_from, board_view, piece_view):
    if move_from == None:
        if (
            chr(ord(move_to[0]) + 1) + EN_PASSANT_ROW_TO[move_to[-1]]
            in piece_view[piece]
        ):
...
def make_pawn_move(move, board_view, piece_view):
    ...
    if is_enpassant(move, move_to, board_view):
        board_view, piece_view = make_en_passant(
            piece, move_to, move_from, board_view, piece_view
        )

    elif is_capture(move):
        board_view, piece_view = make_capture(
            piece, move_to, move_from, board_view, piece_view
        )

    else:
        board_view, piece_view = make_normal_move(
            piece, move_to, board_view, piece_view
        )

    if is_promotion(move):
        promote_to = move[-1]
        board_view, piece_view = promote(
            piece, move_to, promote_to, board_view, piece_view
        )
