...
def can_rook_move(from_pos, to_pos, board_view):
    if from_pos[0] == to_pos[0]:
        col = from_pos[0]
        return all(
            board_view[col + chr(row)] == SPACE
            for row in range(ord(from_pos[1]) + 1, ord(to_pos[1]))
        )

    elif from_pos[1] == to_pos[1]:
        row = from_pos[1]
        return all(
            board_view[chr(col) + row] == SPACE
            for col in range(ord(from_pos[0]) + 1, ord(to_pos[0]))
        )
...
def can_bishop_move(from_pos, to_pos, board_view):
    if abs(ord(from_pos[0]) - ord(to_pos[0])) == abs(ord(from_pos[1]) - ord(to_pos[1])):
        ...
        return all(
            board_view[chr(col) + chr(row)] == SPACE
            for col, row in zip(
                range(ord(from_pos[0]) + change_col_by, ord(to_pos[0]), change_col_by),
...
def can_knight_move(from_pos, to_pos, board_view):
    return (
        abs(ord(from_pos[0]) - ord(to_pos[0])) == 2
        and abs(ord(from_pos[1]) - ord(to_pos[1])) == 1
    ) or (
        abs(ord(from_pos[1]) - ord(to_pos[1])) == 2
        and abs(ord(from_pos[0]) - ord(to_pos[0])) == 1
    )
    ...
    if from_pos != None:
        ...
        if len(piece_view[piece]) == 1:
            ...
            from_pos = [
                ...
                if can_piece_move(piece, pos, to_pos, board_view)
            ][0]
